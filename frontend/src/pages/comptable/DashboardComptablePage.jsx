...
export default function DashboardComptablePage() {
  ...
  const exporterRecapPDF = () => {
    ...
    doc.setFontSize(14);
    doc.setTextColor(0);
    doc.text("RECAPITULATIF DES VACATIONS", 105, 30, { align: "center" });

    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`ISGE-BF - ${MOIS[mois-1]} ${annee}`, 105, 38, { align: "center" });
    ...
    autoTable(doc, {
      startY: 48,
      head: [["Enseignant", "Periode", "Statut", "Montant brut", "Montant net"]],
      ...
      foot: [[
        ...
        "",
        { content: stats.montantTotal.toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ") + " FCFA", styles: { fontStyle: "bold" } }
      ]],
      headStyles:         { fillColor: [13, 110, 253] },
      footStyles:         { fillColor: [240, 240, 240] },
      alternateRowStyles: { fillColor: [248, 249, 250] },
    ...
    doc.text(`Fiches approuvees : ${stats.approuvees}`, 20, finalY + 8);
    doc.text(`Fiches payees : ${stats.payees}`,        20, finalY + 16);

    doc.setFontSize(13);
    doc.setTextColor(13, 110, 253);
    doc.text(`Montant total net : ${stats.montantTotal.toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")} FCFA`,
      20, finalY + 28);
    ...
    doc.save(`recap-vacations-${MOIS[mois-1]}-${annee}.pdf`);
  };
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        <div>
          ...
          <p style={{ margin: 0, color: "#6b7280", fontSize: "0.875rem" }}>
            Validation et approbation des fiches de vacation
          </p>
        </div>
        {vacations.length > 0 && (
      ...
      {message && (
        ...
        }}>{message} <button onClick={() => setMessage("")} style={{ background: "none", border: "none", cursor: "pointer", float: "right" }}>✕</button></div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: "1.5rem" }}>
        {[
          ...
          { icon: "🏦", label: "Montant net total", value: stats.montantTotal > 0 ? stats.montantTotal.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, " ") + " F" : "0 F", color: "#c27803", bg: "#fef3c7" },
        ].map((k, i) => (
          <div key={i} style={{
      ...
      {loading ? (
        ...
        <div style={{
          ...
          textAlign: "center", color: "#1e429f", border: "1px solid #bfdbfe"
        }}>
          ℹ️ Aucune fiche trouvée. Ajustez les filtres et cliquez sur Chercher.
        </div>
      ) : vacations.map(v => {
        ...
        return (
          <div key={v.id} style={{
            ...
            <div style={{
              ...
              <div>
                <h5 style={{ margin: 0, fontFamily: "'Poppins',sans-serif", color: "#111928" }}>
                  {v.enseignant_nom}
                </h5>
            ...
            {v.alertes && v.alertes.length > 0 && (
              <div style={{
                ...
                <strong style={{ color: "#92400e", fontSize: "0.875rem" }}>⚠️ Alertes :</strong>
                {v.alertes.map((a, i) => (
                  <div key={i} style={{ color: "#78350f", fontSize: "0.8rem", marginTop: 2 }}>• {a}</div>
                ))}
              </div>
            )}
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem" }}>
                  <thead>
                    <tr style={{ background: "#f9fafb" }}>
                      {["Matière","Semaine","Durée (h)","Taux (FCFA)","Montant","Pointage QR","Cahier"].map(h => (
                  ...
                  <tbody>
                    {v.lignes.map((l, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid #f3f4f6" }}>
                        ...
                        <td style={{ padding: "8px 10px", fontWeight: 600, color: "#1a56db" }}>{formatMontant(l.montant)}</td>
                        ...
                        <td style={{ padding: "8px 10px" }}>
                          <span style={{
                            ...
                            borderRadius: 20, padding: "2px 8px", fontSize: "0.7rem", fontWeight: 600
                          }}>{l.statut_cahier === "cloture" ? "✅ Clôturé" : "⏳ En cours"}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr style={{ background: "#f9fafb", fontWeight: 700 }}>
                      ...
                      <td style={{ padding: "8px 10px" }}>
                        {formatNombre(v.lignes.reduce((s, l) => s + parseFloat(l.duree_heures || 0), 0))}h
                      </td>
                      <td style={{ padding: "8px 10px" }}></td>
                      <td style={{ padding: "8px 10px", color: "#1a56db" }}>{formatMontant(v.montant_brut)}</td>
                      <td colSpan={2}></td>
                    </tr>
                  </tfoot>
                </table>
